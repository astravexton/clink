...
sub on_public {
    ...
    if ( $arg =~ /^\.choose (.*,.*)/) {
        my @split = split(/,/, $1);
        $conn->privmsg(@to, "$nick: @split[rand @split]");
    }
    # Here's where we want to "parse" channel text
    if ( $arg =~ /$cfg_nick:? (hello|hi|hey)/ ) {
        $conn->privmsg(@to, "Hello $nick!");
    }
    print "<$nick> $arg\n";
...
sub on_invite {
    ...
    if ( $host eq "meetmehereonirclo.lol" or $host eq "antispammeta/suchmeta/doge" ) {
        $conn->join($arg);
    }
}
